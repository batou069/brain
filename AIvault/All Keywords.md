...
```mermaid
graph TD
    A[Phase 1: Problem Framing] --> B{What is the Business Goal?};
    B --> C[Translate to ML Problem];
    C --> D{Supervised or Unsupervised?};
    D -- Labeled Data --> E[Classification / Regression];
    D -- Unlabeled Data --> F[Clustering / Anomaly Detection];
    E --> G[Select Metrics: Accuracy, F1, AUC, RMSE, MAE];
    F --> G;

    G --> H[Phase 2: Data Exploration & Prep];
    H --> I[EDA: Univariate, Bivariate Analysis];
    I --> J[Check for Missing Values, Outliers, Correlation];
    J --> K{How to Preprocess?};
    K --> L[Handle Missing Data, Encode Categoricals, Scale Numerics];
    L --> M[Feature Engineering & Selection];

    M --> N[Phase 3: Modeling & Evaluation];
    N --> O[Establish Simple Baseline Model];
    O --> P{Choose Candidate Models};
    P --> Q[Linear Models, Tree Ensembles, KNN, etc.];
    Q --> R{Select Validation Strategy};
    R -- General --> S[k-Fold Cross-Validation];
    R -- Time-Series Data --> T[Time Series Split];
    S --> U[Tune Hyperparameters];
    T --> U;

    U --> V[Phase 4: System Design & Deployment];
    V --> W{Inference: Batch or Real-time?};
    W --> X[Design System Architecture: API, Feature Store];

    X --> Y[Phase 5: Post-Deployment];
    Y --> Z[Monitor: Data Drift, Model Performance];
    Z --> AA[Define Retraining Strategy];
```
